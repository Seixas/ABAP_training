...
*      Are we only to find customer functions
      ...
      IF skipthisloop = false.
        ...
        REPLACE ALL OCCURRENCES OF '''' IN wafunction-functionname
        WITH ' '.
...
*-----------------------------------------------------------------------
*  scanForFunctionIncludes... Find all user defined includes within
*the function group
*-----------------------------------------------------------------------
...
* Once we have a list of all the includes we need to loop round them
*  an select all the other objects
  LOOP AT ilocprogram INTO waprogram.
    mytabix = sy-tabix.
...
* Now we have all the program includes and details we need to find
*extra functions
...
*-----------------------------------------------------------------------
*   findMainMessageClass... find the message class stated at the top
* of  program.
*-----------------------------------------------------------------------
...
*     Select all classes
      ...
      IF sy-subrc <> 0.
        SELECT clsname descript msg_id
               ...
               WHERE clsname IN rangeclass
               AND langu = language
                 AND ( author IN rangeauthor OR changedby IN
        rangeauthor )
                 AND version = '0'
                 AND ( state = '0' OR state = '1' ).
      ENDIF.
...
*  Once we have a list of all the classes we need to loop round them
*    an select all the other objects
    mytabix = sy-tabix.
...
*   If the class has specified a message class but were unable to find
*   any specific messages then retrieve the whole message class.
    IF ( NOT waclass-msg_id IS INITIAL AND waclass-imessages[] IS
INITIAL ).
...
dumiinterfaces[]
  ...
  LOOP AT iintmethod.
    CONCATENATE iintmethod-interfacename '~' iintmethod-cmpname INTO
    wamethod-cmpname.
...
FORM findclassmethods USING value(classname)
  ...
  DATA: iredefinedmethods TYPE STANDARD TABLE OF seoredef WITH HEADER
  LINE.
...
*  For Each method we must find the original class the method was
*  created in
...
*-----------------------------------------------------------------------
* findRedefinitionClass... find the original class the method was
*redefined from
*-----------------------------------------------------------------------
FORM findredefinitionclass USING value(redefinedclassname) TYPE
seoclsname
...
*-----------------------------------------------------------------------
* findRedefinitionClassRecur... Recursively find the original class
*the method was redefined from
*-----------------------------------------------------------------------
FORM findredefinitionclassrecur USING value(redefinedclassname) TYPE
seoclsname
                                 value(methodname) TYPE seocpdname
                                       originalclassname TYPE
seoclsname.
...
FORM findmainfunctioninclude USING value(programname)
  ...
  CONCATENATE namespace 'L' functiongroup 'U' functionincludeno INTO
  functionincludename.
ENDFORM.                               "checkComboBoxes
...
* quicker than converting it again
      ...
      IF sy-subrc <> 0.
        CONCATENATE 'Converting table' <wadictionary>-tablename
...
*     Find the text for this concept
      CALL FUNCTION 'SOTR_GET_TEXT_KEY'
        EXPORTING concept = <waconcept>-concept
                  langu = pmlang
...
* Add a html header to the table
  ...
  APPEND `    <tr>` TO ilochtml.
  APPEND `      <td><!--This is where our main table begins  -->` TO
  ilochtml.
...
* Prgroamname and description
  ...
  CONCATENATE `     <td><h2>Code listing for: ` programname `</h2>`
  INTO htmltable.
  APPEND htmltable.

  CONCATENATE `<h3> Description: ` shortdescription `</h3></td>` INTO
  htmltable.
  APPEND htmltable.
...
* Class name and description
  ...
  CONCATENATE `     <td><h2>Code listing for class: ` classname
  `</h2>` INTO htmltable.
  APPEND htmltable.
...
* Class name and description
  ...
  CONCATENATE `     <td><h2>Code listing for function ` functionname
  `</h2>` INTO htmltable.
  APPEND htmltable.
...
* to remove forward slashes
    ...
    CONCATENATE copyofcurrentline 'global-' lowercaselink  "functionName
            period htmlextension '">' 'Global data declarations' '</a>'
           INTO copyofcurrentline.

              CONCATENATE copyofcurrentline '</div><br />' INTO
              copyofcurrentline.
                  ...
                  TRY.
                   ...
                   'y' OR tail+0(1) = 'z' OR tail CS customernamerange.
                      ...
                      copyofcurrentline = 'include structure <a href ='.
                        ...
                        lowercaselink = hyperlinkname.
                        TRANSLATE lowercaselink TO LOWER CASE.
*                   If we are running on a non UNIX environment we
*will need to remove forward slashes
                        IF frontendopsystem = non_unix.
                          TRANSLATE lowercaselink USING '/_'.
                        ENDIF.
...
*to iLocHeader.
  CONCATENATE '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.1//EN" '
              '"http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd">'
         INTO waheader.
...
*append '<meta http-equiv="Content-Type" content="application/xhtml+xml;
* charset=ISO-8859-1" />' to iLocHeader.
...
*append `.code{ font-family:"Courier New", Courier, monospace;
* color:#000; font-size:14px; background-color:#F2F4F7 }` to iLocHeader.
  CONCATENATE `.code{ font-family:"Courier New", Courier, monospace; `
              `color:#000; font-size:14px; background-color:#F2F4F7 }`
         INTO waheader.
  APPEND waheader TO ilocheader.
  CLEAR waheader.

*append `  .codeComment {font-family:"Courier New", Courier, monospace;
* color:#0000F0; font-size:14px; background-color:#F2F4F7 }`
* to iLocHeader.
  CONCATENATE `  .codeComment {font-family:"Courier New", Courier, `
              `monospace; color:#0000F0; font-size:14px; `
              `background-color:#F2F4F7 }`
         INTO waheader.
  APPEND waheader TO ilocheader.
  CLEAR waheader.

*append `  .normalBold{ font-family:Arial, Helvetica, sans-serif;
* color:#000; font-size:12px; font-weight:800 }` to iLocHeader.
CONCATENATE `  .normalBold{ font-family:Arial, Helvetica, sans-serif; `
            `color:#000; font-size:12px; font-weight:800 }`
       INTO waheader.
  APPEND waheader TO ilocheader.
  CLEAR waheader.

*append `  .normalBoldLarge{ font-family:Arial, Helvetica, sans-serif;
* color:#000; font-size:16px; font-weight:800 }` to iLocHeader.
  CONCATENATE `  .normalBoldLarge{ font-family:Arial, Helvetica, `
            `sans-serif; color:#000; font-size:16px; font-weight:800 }`
         INTO waheader.
  APPEND waheader TO ilocheader.
  CLEAR waheader.
...
FORM addtablestyles USING ilocheader LIKE dumihtml[]
  ...
  APPEND `     font-family:"Courier New", Courier, monospace;` TO
  ilocheader.
...
*append `  .normal{ font-family:Arial, Helvetica, sans-serif;
* color:#000; font-size:12px }` to iLocHeader.
  CONCATENATE `  .normal{ font-family:Arial, Helvetica, sans-serif; `
              `color:#000; font-size:12px }`
         INTO waheader.
  APPEND waheader TO ilocheader.
  CLEAR waheader.

*  append `  .footer{ font-family:Arial, Helvetica, sans-serif;
* color:#000; font-size:12px; text-align: center }` to iLocHeader.
  CONCATENATE `  .footer{ font-family:Arial, Helvetica, sans-serif; `
              `color:#000; font-size:12px; text-align: center }`
         INTO waheader.
  APPEND waheader TO ilocheader.
  CLEAR waheader.

*  append `  h2{ font-family:Arial, Helvetica, sans-serif; color:#000;
*font-size:16px; font-weight:800 }` to iLocHeader.
  CONCATENATE `  h2{ font-family:Arial, Helvetica, sans-serif; `
              `color:#000; font-size:16px; font-weight:800 }`
         INTO waheader.
  APPEND waheader TO ilocheader.
  CLEAR waheader.

*  append `  h3{ font-family:Arial, Helvetica, sans-serif; color:#000;
*font-size:14px; font-weight:800 }` to iLocHeader.
  CONCATENATE `  h3{ font-family:Arial, Helvetica, sans-serif; `
...
*-----------------------------------------------------------------------
* buildFooterMessage...Returns a footer message based on the output
*file type.
*-----------------------------------------------------------------------
FORM buildfootermessage USING returnmessage.
  ...
  CONCATENATE `Extracted by Mass Download version `
              ...
              sy-datum+0(4) `. Sap Release ` sy-saprl INTO
  returnmessage.
ENDFORM.                               "checkComboBoxes
                                       "buildFooterMessage
...
*   Description
    ...
    watreedisplay-tcolor3    = 1.
    watreedisplay-tpos3      = 60.
    CONCATENATE `Class:    ` <waclass>-descript INTO
    watreedisplay-text3.
    APPEND watreedisplay TO itreedisplay.
